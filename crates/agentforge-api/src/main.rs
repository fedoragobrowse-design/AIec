...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tenant_id: Uuid = required("AGENTFORGE_TENANT_ID")?.parse()?;
    let runtime_name = required("AGENTFORGE_RUNTIME")?;
    ...
    let storage_scheduler: Arc<dyn agentforge_storage::Scheduler> =
        Arc::new(PostgresScheduler::new(postgres));
    let scheduler: Arc<dyn Scheduler> = Arc::new(StorageScheduler::new(
        repository.clone(),
        storage_scheduler,
        std::env::var("AGENTFORGE_LEASE_TTL_SECONDS")
            .ok()
    ...
    let runtime: Arc<dyn SandboxRuntime> =
        Arc::new(WorkerRuntime::new(worker_client, scheduler.clone()));
    let state = AppState::new(repository.clone(), runtime)
